...
in {
  ...
  home-manager.users.andrei = { pkgs, ... }: {
    ...
    xdg.configFile."hypr/dropdown.conf".text = ''
      ...
      # Using windowrule for Hyprland 0.51.x compatibility

      # Assign to special workspace FIRST (before other rules)
      windowrule = workspace special:dropdown silent, class:^(dropdown)$

      # Float the dropdown window
      windowrule = float, class:^(dropdown)$

      # Golden ratio size (61.8% width and height)
      windowrule = size ${dropdownWidth} ${dropdownHeight}, class:^(dropdown)$

      # Center horizontally, position one bar height below the bar (${toString barHeight}*2 = ${toString (barHeight * 2)}px)
      windowrule = move ${dropdownX} ${toString (barHeight * 2)}, class:^(dropdown)$

      # Visual styling
      windowrule = dimaround, class:^(dropdown)$
    '';
  };
}
