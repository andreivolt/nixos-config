...
myScratchpads =
  [ NS "music"  "google-play-music-desktop-player" (className =? "Google Play Music Desktop Player") $ (customFloating myCenterFloatRect)
  , NS "irc"    "irc"                              (title =? "irc") $ (customFloating myTopFloatRect)
  , NS "mail"   "email"                            (title =? "mail") $ (customFloating myTopFloatRect)
  ]
  where
