float ease(float x) {
    return pow(1.0 - x, 3.0); // Reduced to 3.0 for even faster falloff
}
...
const float DURATION = .075; // Reduced to 0.075 for 2x faster trail
const float OPACITY = .4; // Increased to 0.4 for better visibility with faster animation
