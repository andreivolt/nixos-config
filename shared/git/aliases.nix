{
  ...
  ap = "add --patch";
  l = "log --exclude='refs/original/*' --all --graph --format=format:'%C(bold blue)%h%C(bold yellow)%d%C(reset) - %C(bold green)%<(14)%ar%C(reset) %s %C(dim white)- %<(12,trunc)%an%C(reset)'";
